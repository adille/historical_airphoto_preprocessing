...
Authors: Antoine DILLE for version 1.0.1
...
print('=         Version 1.0.1 (December 2021)  |  A. Dille (RMCA/VUB)     =')
print('=====================================================================')
print(' ')
...
if tool == 'opencv':
    ...
    outimlist = outfiles + [filename for filename in outfiles if filename[-5:] in [".tiff", ".TIFF"]]
...
# B. Downscaling with Pillow !!! need to be improved
def Pillow():
    ...
    for image in imlist:
        ...
        scale_percent = 60  # percent of original size 60% 1500dpi=900 dpi
        width = int(img.size[1] * scale_percent / 100)
        height = int(img.size[0] * scale_percent / 100)
        dim = (width, height)
        # resize image
        resized = img.resize(dim, resample=Image.BICUBIC)
        ...
        # # Save
        # OutFolder=image_folder + '/Downscaled'
        # Path(OutFolder).mkdir(parents=True, exist_ok=True)
        # resized_name= image[:-4] + "_Downscaled" + extension
        # # Filename
        # downSname = OutFolder + '/' + resized_name
        ...
        # strenght = 0.9
        # # Median filtering
        # resized_median = median_filter(resized, 1)
        # # Calculate the Laplacian (edge detector)
        # lap = cv2.Laplacian(resized_median, cv2.CV_64F)
        # # Calculate the sharpened image
        # sharpened = resized - strenght * lap
        ...
        # sharpened=unsharp_mask_OpenCV(resized, kernel_size=(3, 3), sigma=1.0, amount=1.0, threshold=0)
        sharpened=unsharp_mask_Pillow(resized, Inradius=3)
        # Save
        OutFolder=WorkDir + '/Downscaled'
        Path(OutFolder).mkdir(parents=True, exist_ok=True)
        resized_name= image[:-4] + "_DownSharp" + extension
        # Filename
        downSname = OutFolder + '/' + resized_name
        ...
        # Using cv2.imwrite() method
        # Saving the image
        cv2.imwrite(downSname, sharpened)
...
# C. Downscaling with scikit !!! need to be improved
def scikit():
    from skimage.io import imread, imsave
    from skimage.filters import unsharp_mask
    from skimage.transform import resize
    for image in imlist:
        img = imread(WorkDir + '/' + image)
        print('Original Dimensions : ', img.shape)
        ...
        # resize image
        resized = resize(img,dim, order=3) #3: Bi-cubic

        # sharpen
        sharpened = np.copy(resized).astype(np.uint16)
        for i in range(3):
            sharpened[..., i] = unsharp_mask(resized[..., i], radius=2, amount=2)
