...
# ----------------------------------------------------------------------------
# input: dataset
dataset='Luluaburg_1959' # image dataset name (e.g., 'Virunga_1958'). this variable is include in the template name (e.g., Template_Virunga_1958_top_left_1.tif)

# input: images
image_folder = r"E:\PROCESS\SCANs\SCANS_Luluaburg\_Canvas_Sized" # where are located the images to process

# input: fiducial templates
fiducial_template_folder = r"C:\Users\adille\Desktop\Tests\SCANs\Fiducials_Barriere\Luluaburg_1959\Fiducial_templates_Luluaburg_1959" # folder with the template images
center_fidu_tempate_CSV= fiducial_template_folder + "/Center_Fiduciales.txt" #text file where the centre of the template is indicated
'''
...
# outputs
corner_folder=r"C:\Users\adille\Desktop\Tests\SCANs\Fiducials_Barriere\Luluaburg_1959\_temp_corners_Luluaburg_1959" # folder where temporary fiducials of the images (i.e., where
...
def addLine(image_name, Fiducial_Coordinates):
    """
...
def CenterFiducial_LUCASKANADE(img2,Fidu_type,orientation,template,xc,yc,image_name, corner,type_fidu):
     """
...
def FiducialFig(F,fidu_coordinates):
    """
...
def Main(image_name, S, p, Fidu_type, black_stripe_location,type_fidu):
            ...
            if xc !=0 and yc != 0: # i.e., it found a template
                try :
                    if Fiducial_type=='target' :
                        orient='False'
                        u,v, maxVal = CenterFiducial_LUCASKANADE(F[corner][0],Fiducial_type,orient,template_dic[template_name],xc,yc,image_name,corner,type_fidu)
                        ...
                        if len(Coord) == 4 and template_name==template_list[-1] and corner == list(F.keys())[-1]:
                            ...
                            addLine(image_name, Coord) # Add to CSV file

                            FiducialFig(F, fidu_coordinates) # save a figure
...
if RunParallel is True:
    Parallel(n_jobs=num_cores, verbose=30)(delayed(Main)(image,S,p,Fiducial_type,black_stripe_location,type_fidu) for image in imlist)
else:
    count=1
    for image in imlist:
        print('\n >>> Image [' + str(count) + '/' + str(len(imlist)) + ']: ' + image)
        Main(image, S, p, Fiducial_type, black_stripe_location, type_fidu)
        count=count +1
